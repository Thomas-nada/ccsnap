...
function createEl(tag, attrs = {}, children = []) {
  ...
  Object.entries(attrs).forEach(([key, value]) => {
    if (key === 'class') {
      ...
      // FIX: ensure event name is lowercase for addEventListener
...
function copyToClipboard(text) {
  // Use modern clipboard API first (requires secure context: https or localhost)
  if (navigator.clipboard && window.isSecureContext) {
    navigator.clipboard.writeText(text).then(() => {
      // Replaced alert() with a console log or a less intrusive message
      ...
      // Fallback if modern API fails (e.g., due to permission or context)
      fallbackCopyTextToClipboard(text);
    });
  } else {
    // Fallback for older browsers or unsecured contexts (like file:// or http://)
    fallbackCopyTextToClipboard(text);
  }
}

// Legacy/Fallback clipboard logic
function fallbackCopyTextToClipboard(text) {
  const textarea = document.createElement('textarea');
  textarea.value = text;
  // Make it non-editable to avoid focus/scrolling side effects
...
// Updated: Shows only Token with Copy button
function openSuccessModal(message, entryId = null, editToken = null) {
  ...
  if (editToken) {
    ...
    // Entry ID for reference (optional, display only)
    if (entryId) {
...
if (btnLoadManual) {
  btnLoadManual.addEventListener('click', () => {
    ...
    if (!token) {
      showMessage('Please enter your Secret Token.', 'error'); // Updated message
      return;
    }
    ...
    .catch(err => {
      ...
      showMessage('Could not find application with that token.', 'error'); // Updated message
    });
  });
}
