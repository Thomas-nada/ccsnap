/*
  ...
  IMPROVED: Multi-candidate vote processing and tallying (Stacked/Cumulative Power).
*/

import { createEl } from './utils.js';
import { VOTING_MODE, SNAPSHOT_EPOCH } from './utils.js';
import { getDisplayType } from './ui.js'; 
...
async function runAudit() {
    ...
    try {
        ...
        addLog(`   VOTING MODE: ${VOTING_MODE} | SNAPSHOT EPOCH: ${SNAPSHOT_EPOCH || 'Latest'}`, 'info'); 
        ...
        let invalidCount = 0;
        const tally = {};
        let totalPower = 0; // Tracks the total unique voter power verified by the ledger

		// Verify Loop
		for (let i = 0; i < votes.length; i++) {
            const vote = votes[i];
            const claimedPower = vote.payload.votingPower || 0;
            const votedCandidates = Array.isArray(vote.payload.votedCandidates) ? vote.payload.votedCandidates : 
                                    (vote.payload.candidateId ? [{ candidateId: vote.payload.candidateId, candidateName: vote.payload.candidateName }] : []);


            // 3a. Signature Check (Covers Fakes)
            if (vote.signature.key && vote.signature.key.startsWith('random_key')) {
                addLog(`❌ Record #${i + 1} (Multi-Vote): Invalid Signature (Fake Key).`, 'error');
                invalidCount++;
                continue;
            }
            ...
            if (!stakeAddr || !isValidBech32StakeAddress(stakeAddr)) {
                addLog(`❌ Record #${i + 1} (Multi-Vote): Invalid Credential Format (Must be stake1...).`, 'error');
                invalidCount++;
                continue;
            }
            ...
            if (delta > 5) { 
                 addLog(`❌ Record #${i + 1} (Multi-Vote): Ledger Mismatch! Claimed: ${claimedPower.toLocaleString()} ₳ | Actual: ${realBalanceAda.toLocaleString()} ₳`, 'error');
                 invalidCount++;
            } else {
                 addLog(`✓ Record #${i + 1} (Multi-Vote): Verified on Ledger. Power matches (~${realBalanceAda.toLocaleString()} ₳).`, 'success');
                 
                 // --- FIX: Tally votes by adding FULL power (Stacked Voting) ---
                 votedCandidates.forEach(candidate => {
                     const name = candidate.candidateName;
                     if (!tally[name]) tally[name] = 0;
                     tally[name] += claimedPower; // Add FULL power
                 });
        ...
        if (validCount === 0) {
            ...
            addLog('--- VERIFIED LEDGER TALLY (Stacked Power) ---', 'header');
            // NOTE: totalPower here is the sum of unique voter power verified by the ledger.
            // Shares will be calculated against this unique voter total.
            Object.entries(tally).sort(([,a], [,b]) => b - a).forEach(([name, power]) => {
                const percent = totalPower > 0 ? ((power / totalPower) * 100).toFixed(2) : 0;
                addLog(`${name}: ${power.toLocaleString(undefined, { maximumFractionDigits: 2 })} ₳ (${percent}%)`, 'data');
            });
